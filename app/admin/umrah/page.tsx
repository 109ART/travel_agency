...
export default async function UmrahRequestsPage({ searchParams }: { searchParams: Promise<{ status?: string }> }) {
  ...
  return (
    <>
      ...
      <DataTable head={["Request", "Customer", "Trip details", "Documents", "Quotation", "Supplier cost", "Status", "Payment"]} min={1350}>
        {rows.length === 0 && <EmptyRow cols={8} text="No Umrah requests here yet." />}
        {rows.map((r) => {
          const emailLink = mailtoLink(
            r.user.email,
            `Your SafarPro Umrah request ${r.reference}`,
            `Assalam o Alaikum ${r.user.name},\n\nYour Umrah request (${r.reference}) is currently: ${label(r.status)}.\n${
          ...
          return (
            <tr key={r.id} className="align-top transition hover:bg-slate-50">
              ...
              <td className="space-y-2 px-4 py-3">
                ...
                <CostForm kind="umrah" id={r.id} costPrice={r.costPrice} supplierName={r.supplierName} />
              </td>
              <td className="px-4 py-3">
